...
if ($_SESSION['role'] != 'user') {
  ...
  if (isset($_POST['submit'])) {
    ...
    $fromdepartment=$_POST['department'];
    ...
    $query = mysqli_query($conn, "INSERT INTO tblcomplaints(userId,complaintType, complainasset, ExpiryDate, complaintDetails, fromdepartment, todepartment, departmentcode, state, location, place, branchcode, complaintFile) VALUES ('$uid','$complaintype', '$complainasset', '$Description', '$complaintdetials', '$fromdepartment', '$todepartment', '$departmentcode', '$state', '$location', '$place', '$branchcode', '$compfile')");
    if ($query) {
...
<body>
        <div class="flex flex-col flex-auto flex-shrink-0 min-h-screen antialiased text-black bg-zinc-100 ">
            <div class="h-full  mb-20 ml-64 mt-14 md:ml-64 md:px-20 xl:px-12" >
                <div class="h-2 mt-10 bg-pink-500 rounded-t-md"></div>
                ...
                <form class="min-w-full p-10 pl-10  bg-white rounded-lg shadow-xl xl:px-10" method="post" action="" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-3">
                        ...
                        <div>
                            <label class="block my-3 font-semibold text-gray-800 text-md" for="email">From Department</label>
                            <select type="text" name="department" id="department" onChange="getDep(this.value);" class="w-full px-4 py-2 rounded-lg bg-zinc-100 focus:outline-none">
                        <!-- <option value="">Select Department</option> -->
                        <option value="<?php echo $_SESSION['department']?>"><?php echo $_SESSION['department']?>
                                </option>
                        ...
                        <div class="grid grid-cols-3 place-content-center">
                            <div>
                                ...
                                <input  type="file" name="compfile" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-pink-500 file:text-amber-50 " value=""/>
                            </div>
                        </div>
