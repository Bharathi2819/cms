...
if(isset($_GET['del']))
		  {
		          mysqli_query($conn,"delete from master_branches where id = '".$_GET['id']."'");
...
<body>
            <div class="h-full  mb-50 ml-64 mt-16 md:ml-64 md:px-20 xl:px-12 " >
                ...
                <form class="min-w-full p-10 pl-100 mb-10 bg-white rounded-lg shadow-xl " method="post" action="" >
                    <h1 class="mb-10 font-sans text-2xl font-bold text-center uppercase text-sky-800 ">Manage Facility</h1>
                    ...
                    <div class="grid grid-cols-2 gap-4 xl:grid-cols-2">
                        <div>
                            <a href="adfac.php" type="button" class="text-white bg-gradient-to-r from-pink-400 via-pink-500 to-pink-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-pink-300 font-medium rounded-lg text-base px-10 py-3 text-center mr-2 mb-2">Add</a>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <div class="w-full lg:w-6/6">
                                ...
                                <table class="w-full table-auto min-w-max">
                                    <thead>
                                        <tr class="text-base leading-normal text-white uppercase bg-sky-800">
                                            ...
                                            <th class="px-6 py-3 text-start">State</th>
                                            <th class="px-6 py-3 text-start">City</th> 
                                            <th class="px-6 py-3 text-start">Location</th>
                                            <th class="px-6 py-3 text-start">Branch_Code</th>
                                            <th class="px-6 py-3 text-start">Branch_Address</th> 
                                            <th class="px-6 py-3 text-start">Creation Date</th>
                                            <th class="px-6 py-3 text-start">last Updation date</th>
                                            <th class="px-6 py-3 text-start">Actions</th>
                                        </tr>
                                    </thead>
                                    <?php $query=mysqli_query($conn,"select * from master_branches LIMIT $start_from, $per_page_record ");
                                        $cnt=1;
                                    ...
                                    <tbody class="text-base  text-black font-semibold  even:bg-white-100 odd:bg-sky-50">
                                        <tr >
											...
											<td class="px-6 py-3 text-start" ><?php echo htmlentities($row['branch_state']);?></td>
											<td class="px-6 py-3 text-start"><?php echo htmlentities($row['branch_city']);?></td>
											<td class="px-6 py-3 text-start"> <?php echo htmlentities($row['branch_name']);?></td>
											<td class="px-6 py-3 text-start"><?php echo htmlentities($row['branch_code']);?></td>
                                            <td class="px-6 py-3 text-start" ><?php echo htmlentities($row['branch_address']);?></td>
											<td class="px-6 py-3 text-start"><?php echo htmlentities($row['created_at']);?></td>
                                            <td class="px-6 py-3 text-start"><?php echo htmlentities($row['updated_at']);?></td>
                                            <td class="px-6 py-3 text-start">
                                                <div class="flex justify-center item-center">
                                                    <a href="facedit.php?id=<?php echo $row['id']?>" class="w-5 mr-4 transform text-green-500 hover:text-green-800 hover:scale-110">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                        </svg>
                                                    </a>
                                                    <a href="addfacility.php?id=<?php echo $row['id']?>&del=delete" onClick="return confirm('Are you sure you want to delete?')" class="w-5 mr-2 transform text-red-500 hover:text-red-800 hover:scale-110">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="flex justify-center mt-10">
                                ...
                                $total_records_query = "SELECT COUNT(*) FROM state ";
