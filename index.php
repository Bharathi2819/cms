...
if ($count == 1) {
    ...
    echo "departmentcode: " . $row['departmentcode'] . "<br>";
    ...
    if ($row['role'] == 'user') {
        if ($row['emp_id'] == NULL) {
            echo "Redirecting to user profile page<br>";
            header("Location:./users/profile.php");
        } else {
            echo "Redirecting to user dashboard page<br>";
            header("Location:./users/dashboard.php?emp_id=" . $row['emp_id']);
        }
    } else if ($row['role'] == 'admin') {
        if ($row['emp_id'] == NULL) {
            echo "Redirecting to admin profile page<br>";
            header("Location:./users/tprofile.php");
        } else {
            echo "Redirecting to admin dashboard page<br>";
            header("Location:./users/tdashboard.php?emp_id=" . $row['emp_id']);
        }
    }
} else {
    $msg = '<script type="text/javascript">alert("Invalid email or password");</script>';
    echo $msg;
}
      ...
      <div class="flex items-center justify-center px-4 py-10 bg-white sm:px-6 lg:px-8 sm:py-16 lg:py-24 xl:h-screen">
        <div class="xl:w-full xl:h-full xl:max-w-sm 2xl:max-w-md xl:mx-auto">
          ...
          <div class="mt-3 space-y-10">
            <a href="admin/index.php" class="relative inline-flex items-center justify-center w-full px-4 py-4 text-base font-semibold text-gray-700 transition-all duration-200 bg-white border-2 border-gray-200 rounded-md hover:bg-gray-100 focus:bg-gray-100 hover:text-black focus:text-black focus:outline-none">
